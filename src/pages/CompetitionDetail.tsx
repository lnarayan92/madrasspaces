import { useParams, Link } from "react-router-dom";
import { useEffect } from "react";
import { ArrowLeft } from "lucide-react";
import Header from "@/components/Header";
import { getCompetitionById } from "@/data/competitions";

const CompetitionDetail = () => {
  const { competitionId } = useParams<{ competitionId: string }>();
  const competition = competitionId ? getCompetitionById(competitionId) : undefined;

  // Scroll to top when page loads
  useEffect(() => {
    window.scrollTo(0, 0);
  }, [competitionId]);

  if (!competition) {
    return (
      <div className="min-h-screen bg-background">
        <Header />
        <div className="pt-32 px-4 text-center">
          <h1 className="text-2xl font-light mb-4">Competition not found</h1>
          <Link
            to="/#competitions"
            className="text-muted-foreground hover:text-foreground transition-colors"
          >
            ← Back to Competitions
          </Link>
        </div>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-background">
      <Header />

      <main className="pt-20 sm:pt-24">
        {/* Back Link */}
        <div className="container mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-8">
          <Link
            to={`/#competition-tile-${competition.id}`}
            className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors duration-300"
          >
            <ArrowLeft className="h-4 w-4" />
            Back to Competitions
          </Link>
        </div>

        {/* Competition Header */}
        <div className="container mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 mb-12 sm:mb-16">
          <p className="text-sm uppercase tracking-widest text-muted-foreground mb-4">
            Competition
          </p>
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-light tracking-tight mb-2">
            {competition.secondaryName}
          </h1>
        </div>

        {/* Basic Info Section */}
        <div className="container mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 mb-12 sm:mb-16">
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4 text-sm">
            <div>
              <p className="text-muted-foreground mb-1">Year</p>
              <p className="font-light">{competition.basicInfo.year}</p>
            </div>
            <div>
              <p className="text-muted-foreground mb-1">Location</p>
              <p className="font-light">{competition.basicInfo.location}</p>
            </div>
            <div>
              <p className="text-muted-foreground mb-1">Type</p>
              <p className="font-light">{competition.basicInfo.type}</p>
            </div>
            <div>
              <p className="text-muted-foreground mb-1">Status</p>
              <p className="font-light">{competition.basicInfo.status}</p>
            </div>
            <div>
              <p className="text-muted-foreground mb-1">Photographers</p>
              <p className="font-light">{competition.basicInfo.photographers}</p>
            </div>
          </div>
        </div>

        {/* Vertical Image Scroll */}
        <div className="container mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 space-y-4 sm:space-y-6">
          {competition.images.map((image, index) => (
            <div
              key={index}
              className="w-full overflow-hidden bg-muted animate-fade-in"
              style={{ animationDelay: `${index * 100}ms` }}
            >
              <img
                src={image.src}
                alt={image.alt}
                className="w-full h-auto object-contain"
                loading={index < 2 ? "eager" : "lazy"}
              />
            </div>
          ))}
        </div>

        {/* Description */}
        <div className="container mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-16 sm:py-20">
          <div className="space-y-6 text-muted-foreground font-light leading-relaxed">
            {competition.description.map((paragraph, index) => (
              <p key={index}>{paragraph}</p>
            ))}
          </div>

          {/* Conclusion */}
          {competition.conclusion && (
            <div className="mt-12 pt-8 border-t border-border">
              <p className="text-muted-foreground font-light leading-relaxed italic">
                {competition.conclusion}
              </p>
            </div>
          )}
        </div>

        {/* Back to Competitions */}
        <div className="container mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 pb-20">
          <Link
            to={`/#competition-tile-${competition.id}`}
            className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors duration-300"
          >
            <ArrowLeft className="h-4 w-4" />
            Back to Competitions
          </Link>
        </div>
      </main>
    </div>
  );
};

export default CompetitionDetail;